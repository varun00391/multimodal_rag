...
from rag_shared.core.deps import CurrentUser
from rag_shared.core.errors import AppError
from rag_shared.models.entities import Document, QueryRecord
from rag_shared.schemas.api import QueryResponse, QuerySource, QueryUsage, RetrievalResultItem, RetrievalSearchResponse
from rag_shared.workspace import document_rag_dir
from rag_shared.services.rag_runtime import TRANSIENT_RETRIEVAL_ERRORS, call_with_retries
...
async def _get_accessible_documents(
    ...
    *,
    ...
    if require_ready:
        from rag_shared.models.enums import DocumentStatus
...
async def run_rag_query(
    ...
    *,
    ...
    if settings.use_microservices_pipeline:
        from rag_shared.clients.pipeline import PipelineClient
        from rag_shared.schemas.internal import GenerationRequest, HybridRetrievalRequest

        client = PipelineClient(settings)
        retrieval = await client.hybrid_retrieval(
        ...
        generation = await client.generate_answer(
            GenerationRequest(query=query, hits=retrieval.hits, style="detailed")
        )
        source_hits = [
            RetrievalHit(
                child=item.child,
        ...
        source_hits = await asyncio.to_thread(
            _retrieve_scoped_hits,
            query,
            children=children,
            parents=parents,
        ...
        answer = await asyncio.to_thread(generate_answer, query, source_hits, rag_settings, style="detailed")
        answer_text = answer.text

    sources = [_hit_to_source(hit) for hit in source_hits]
